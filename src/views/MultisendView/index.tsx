...
export const MultiSenderView: FC = () => {
  ...
  const handleWalletConnection = () => {
    setSendingType(publicKey ? publicKey.toBase58() : '');
  };

  return (
    <div className="container mx-auto max-w-6xl p-8 bg-gray-900 text-white">
      <nav className={`${styles.container} navbar mb-5 shadow-lg bg-gray-800 rounded-lg flex justify-between items-center p-4`}>
        <Link href="/">
          <a className="text-xl font-semibold hover:text-gray-400">SOLANA TOOLS</a>
        </Link>
        <div>
          <WalletMultiButton className="btn mr-2" />
          <ConnectWallet onUseWalletClick={handleWalletConnection} />
        </div>
      </nav>
      ...
      <main>
        <div className="text-center">
          ...
          <p className="text-xl mb-4">
            Supports public address, .sol domains, and Twitter handles
          </p>

          {!sendingType ? (
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {['oneToken', 'oneReceiver', 'csv', 'emergency'].map(type => (
                <li key={type} onClick={() => setSendingType(type)} className="cursor-pointer p-5 rounded-lg bg-gray-700 hover:bg-gray-600 transition-colors">
                  <h4 className="text-3xl font-bold">{optionTitles[type]}</h4>
                  <p className="mt-2">{optionDescriptions[type]}</p>
                </li>
              ))}
            </ul>
          ) : (
...
const optionTitles = {
  oneToken: '1 Token - Multiple Receivers',
  oneReceiver: 'Multiple Tokens - 1 Receiver',
  csv: 'Upload CSV File',
  emergency: 'Emergency Send'
};

const optionDescriptions = {
  oneToken: 'Send one token to multiple receivers',
  oneReceiver: 'Send multiple tokens to one receiver',
  csv: 'Use a CSV file to multi-send tokens and Solana domains',
  emergency: 'Send all of your tokens and NFTs to a new wallet'
};
